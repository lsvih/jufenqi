      ...
      <div class="tab-swiper vux-center content">
        <scroller :height="getScreenHeight()-44+'px'"  scrollbar-y v-ref:lista>
          <div>
            ...
            <div v-else>
              <j-zc-item v-for="group in list0" :data="group"></j-zc-item>
            </div>
          </div>
        </scroller>
      </div>
        ...
        <scroller :height="getScreenHeight()-44+'px'" lock-x scrollbar-y v-ref:listb>
          <div>
            ...
            <div v-else>
              ...
              <j-zc-op-item v-for="order in list1" :data="order"></j-zc-op-item>
            </div>
          </div>
        </scroller>
      </div>
      ...
      <div class="tab-swiper vux-center content">
        <scroller :height="getScreenHeight()-44+'px'" :lock-x="true" scrollbar-y v-ref:listc>
          <div>
            ...
            <div v-else>
              <j-zc-op-item v-for="order in list2" :data="order"></j-zc-op-item>
            </div>
          </div>
        </scroller>
      </div>
      ...
      <div class="tab-swiper vux-center content">
        <scroller :height="getScreenHeight()-44+'px'" :lock-x="true" scrollbar-y v-ref:listd>
          <div>
            ...
            <div v-else>
              <j-zc-op-item v-for="order in list3" :data="order"></j-zc-op-item>
            </div>
          </div>
        </scroller>
      </div>
      ...
      <div class="tab-swiper vux-center content">
        <scroller :height="getScreenHeight()-44+'px'" :lock-x="true" scrollbar-y v-ref:liste>
          <div>
            ...
            <div v-else>
              <j-zc-op-item v-for="order in list4" :data="order"></j-zc-op-item>
            </div>
          </div>
        </scroller>
      </div>
...
<confirm :show.sync="showConfirm.cancel" title="" confirm-text="是" cancel-text="否" @on-confirm="cancel(tempGroupNo)">
...
export default {
  ...
  ready() {
    ...
    // 预约单
    axios.get(`${Lib.C.mOrderApi}materialAppts?filter=customerId:${JSON.parse(localStorage.getItem("user")).userId}|status:1&sort=createdAt,DESC`).then((res) => {
      res.data.data.map((e) => {
        this.list0.push(e)
      })
    }).catch((err) => {
  ...
  methods: {
    ...
    cancel(groupNo) {
      axios.post(`${Lib.C.mOrderApi}materialGroups/${groupNo}/cancel`).then((res) => {
        alert('取消订单成功！')
        location.reload()
      }).catch((res) => {
        alert('取消订单失败，请重试')
    ...
    deleteOrder(groupNo) {
      axios.post(`${Lib.C.mOrderApi}materialGroups/${groupNo}/delete`).then((res) => {
        alert('删除订单成功！')
        location.reload()
      }).catch((res) => {
        alert('删除订单失败，请重试')
